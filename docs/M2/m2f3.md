...
![Rack Unit](assets/rackunit.jpg){ .center width="40%" }
...
## 3. Asignación dinámica de direcciones IP: DHCP

El **Protocolo de Configuración Dinámica de Host (DHCP)** es un protocolo de red que permite a los dispositivos obtener automáticamente una dirección IP y otros parámetros de configuración de red necesarios para comunicarse en una red, como la máscara de subred, la puerta de enlace predeterminada y los servidores DNS. 

Es especialmente útil en redes con un gran número de dispositivos, ya que evita la necesidad de configurar manualmente cada dispositivo con una dirección IP estática. Otra ventaja del DHCP es que permite reconfigurar la red de manera centralizada, ya que los cambios en la configuración de red se pueden realizar en el servidor DHCP sin necesidad de modificar la configuración de cada dispositivo individualmente, por ejemplo, si se cambia la puerta de enlace o los servidores DNS, solo es necesario actualizar la configuración en el servidor DHCP y los dispositivos obtendrán automáticamente la nueva configuración.

En dispositivos inalámbricos, el DHCP es especialmente importante, ya que permite a los dispositivos móviles obtener una dirección IP y conectarse a la red de manera rápida y sencilla sin necesidad de configurar manualmente cada dispositivo.

En el siguiente enlace puedes ver un vídeo explicativo sobre el funcionamiento del DHCP:

- [Vídeo explicativo sobre DHCP](https://youtu.be/0T2eA-01w9Y?si=L3Fr1XOiOtPoAKiU)

!!! task "A208.2(CE1) - Configuración DHCP en Cisco Packet Tracert"

    En esta actividad, deberás configurar un servidor DHCP en el router de tu red para que los dispositivos inalámbricos obtengan automáticamente su dirección IP y otros parámetros de configuración de red.

    Para ello, sigue los siguientes pasos:

    1. Accede a la configuración del router WRT300N y habilita el servicio DHCP.
    2. Configura un rango de direcciones IP que el servidor DHCP pueda asignar a los dispositivos. Asegúrate que el rango no interfiera con el rango de direcciones IP estáticas.
    3. Verifica que los dispositivos inalámbricos obtienen correctamente su dirección IP y otros parámetros de configuración de red mediante DHCP, haciendo ping entre ellos y al router.

## 4. Redes Inalámbricas

Una Red Inalámbrica consiste en un grupo de dispositivos “sin cable” (“dispositivos inalámbricos” o “dispositivos wireless”) que se comunican entre sí a través de ondas electromagnéticas y sin necesidad, por lo tanto, de cableado.

Las Redes de Área Local Inalámbricas, normalmente** se implementan como una extensión de la Red de Área Local cableada de la organización**, para proporcionar movilidad a los usuarios, que no tendrán que estar en un puesto concreto como requiere una conexión física a la red.

El uso de redes inalámbricas ofrecen múltiples ventajas. Entre ellas, el **ahorro de costes que supone no tener que realizar un cableado entre los nodos de la red**, como en las redes cableadas tradicionales. Otra de las ventajas es la **no necesidad de estar en un sitio o puesto concreto para poder conectarse a los recursos de la organización**. 

Siendo evidentes las ventajas que ofrece la tecnología inalámbrica, hay que tener muy en cuenta los riesgos de seguridad adicionales que añaden a los ya existentes en las redes cableadas.

Existen muchos tipos de redes inalámbricas que difieren en arquitectura, tecnología, estándar de comunicación, etc. Nos centraremos en exclusiva a las Redes de Área Local Inalámbricas, también conocidas como WLAN (Wireless Local Area Network) o Redes Wi-Fi. Estas redes inalámbricas se basan en el estándar IEEE 802.11, y será a este tipo, al que se hará referencia a partir de ahora como redes inalámbricas.

### 4.1 Características de las WLAN

Las redes inalámbricas tienen una serie de características que las diferencian de las redes cableadas tradicionales. Algunas de estas características son:

- **Medio de transmisión compartido**. En las redes inalámbricas, el medio de transmisión es compartido por todos los dispositivos que están conectados al mismo punto de acceso. Esto significa que solo un dispositivo puede transmitir datos en un momento dado, por lo que la **velocidad de transmisión efectiva** **de la red podrá verse afectada por la cantidad de dispositivos conectados al mismo punto de acceso**.

- **Atenuación de la señal**. Las señales inalámbricas pueden verse afectadas por obstáculos físicos como paredes, muebles, etc., lo que puede **reducir la calidad de la señal y la velocidad de transmisión de datos**.

- **Saturación del espectro inalámbrico**. Los principales estándares de redes inalambricas operan en las bandas de frecuencia de **2,4 GHz y 5 GHz**. Debido a la popularidad de las redes inalámbricas, estas bandas de frecuencia suelen estar saturadas, lo que puede generar interferencias y reducir el rendimiento de la red.

- **Seguridad**. Las redes inalámbricas **son más vulnerables a ataques de seguridad que las redes cableadas**, ya que las señales inalámbricas pueden ser interceptadas por atacantes que se encuentren dentro del alcance de la señal. Por lo tanto, es fundamental implementar medidas de seguridad adecuadas para proteger la red inalámbrica, como el uso de contraseñas seguras, la encriptación de datos y la autenticación de usuarios.

- **Topologias**. Existen dos tipos de topologías para las redes inalámbricas: la topología **infraestructura** y la topología **ad-hoc**.   
    -- En el **modo Ad Hoc** no existen los AP y los dispositivos cliente se comunican entre sí directamente.   
![Topología Ad Hoc](assets/adhoc.jpg){ .center width="40%" }   

    -- **Modo Infraestructura**, en la que se utilizan AP para conectar los dispositivos cliente entre sí o con el sistema de distribución de red.   
![Topología Infraestructura](assets/infraestructura.jpg){ .center width="40%" }

### 4.2 Elementos y conceptos claves de las redes Wi-Fi

**ESSID - SSID**   
Cada red wirelesss tiene un ESSID (Extended Service Set Identifier), que la identifica, viene a ser la identificación o nombre de la red WIFI. El ESSID consta de cómo **máximo 32 caracteres y es *case-sensitive***. Es necesario conocer el ESSID del AP para poder formar parte de la red WIFI, es decir, el ESSID configurado en el dispositivo móvil tiene que coincidir con el ESSID del AP.

**BBSID**   
Dirección MAC del punto de acceso, la emplean las tarjetas wireless para identificar y asociarse a redes inalámbricas.

**Beacon Frames**   
Los puntos de acceso mandan constantemente anuncios de la red para que los clientes móviles puedan detectar su presencia y conectarse a la red inalámbrica. Estos “anuncios” son conocidos como BEACON FRAMES, si capturamos las tramas de una red inalámbrica podremos ver que normalmente el AP manda el ESSID de la red en BEACON FRAMES, aunque esto se puede deshabitar por software en la mayoría de los AP que se comercializan actualmente.

**Estándares de redes inalámbricas**   
El [IEEE](https://www.ieee.org/) es una organización internacional que desarrolla estándares técnicos en áreas como la informática, las telecomunicaciones, la electrónica y la ingeniería eléctrica. En el ámbito de las redes inalámbricas, el IEEE ha desarrollado una serie de estándares conocidos como **IEEE 802.11**, que definen las especificaciones técnicas para las redes Wi-Fi.

Recursos:

- En el siguiente [enlace](https://www.netspotapp.com/es/blog/wifi-standards/#IEEE_802_11_y_The_Standards_Body) puedes encontrar una tabla comparativa de los principales estándares de redes inalámbricas, incluyendo sus características técnicas, velocidades máximas de transmisión de datos y frecuencias de operación.   
- [Video Explaining Wi-Fi: 802.11 Standards & Generations](https://www.youtube.com/watch?v=QSq1TYTiESA)

**Canales de comunicación**   
Los canales de comunicación en las redes inalámbricas **se refieren a las frecuencias específicas dentro de la banda de frecuencia utilizada por el estándar de red inalámbrica**. En la banda de **2,4 GHz**, por ejemplo, **existen 14 canales** disponibles, aunque en algunos países solo se permiten los canales del 1 al 11. En la banda de 5 GHz, hay muchos más canales disponibles, lo que permite una mayor capacidad y menor interferencia. 

En la siguiente imagen se pueden ver los canales disponibles en la banda de 2,4 GHZ, extraida de [*Data Alliance*](https://www.data-alliance.net/es/canales-wifi-gu%C3%ADa-completa-con-consejos-para-aumentar-el-rendimiento-de-la-se%C3%B1al-usos-ventajas-y-desventajas-de-los-rangos-de-24-ghz-5-ghz-900-mhz?srsltid=AfmBOoq3kqWMQ9umD4bGO3E_lVjUV23yl-oHWNGb8g0iQ4Uxq_vDt3d9)

![Canales de comunicación en la banda de 2,4 GHz](assets/canaleswifi24.jpg){ .center width="50%" }

El ancho de banda de las **redes WiFi de 5GHz** es mucho más amplio que el de las de 2,4GHz, lo que permite tener hasta 24 canales que no se solapan entre sí. 

Otra característica de las redes wifi de 5GHz, es la posibilidad de **combinar canales en 40, 80 y 160MHz**, lo que permite aumentar la velocidad de transmisión de datos, aunque a costa de reducir el alcance de la señal.

**Intensidad de la señal**

Para medir la intensidad de la señal WiFi en un lugar determinado, no se utiliza la medida de vatios, ya que la potencia de la wifi es muy baja y al alejarnos del punto de acceso, la potencia de la señal disminuye rápidamente. En su lugar se utiliza el ***dBm*** (decibelio-miliperiodo) que indica cuánta potencia queda de la señal cuando llega a tu dispositivo.

En redes WiFi, los valores de intensidad de la señal siempre son negativos. Cuanto más cerca de 0, más fuerte es la señal.

La intensidad de la señal en las redes inalámbricas se mide en decibelios (dBm) y se refiere a la potencia de la señal recibida por un dispositivo. 

| Valor (dBm)   | Calidad de señal     | Experiencia de usuario                                                                 |
|----------------|----------------------|-----------------------------------------------------------------------------------------|
| -30 dBm        | Máxima               | Estás pegado al router. Casi imposible de mejorar.                                    |
| -50 a -60 dBm  | Excelente            | Streaming 4K, juegos online sin lag y descargas rápidas.                              |
| -67 a -70 dBm  | Mínimo aceptable     | Lo mínimo para una conexión estable. A partir de aquí, el video empieza a pixelar.    |
| -80 dBm        | Muy mala             | Conexión inestable, cortes frecuentes, solo texto o emails básicos.                   |
| -90 dBm        | Ruido                | El dispositivo ve que hay WiFi, pero no puede conectar. Es "ruido" de fondo.          |


**Relacion Señal-Ruido (SNR)**

Para medir la calidad de la señal WiFi, no solo es importante medir la intensidad de la señal, sino también la cantidad de ruido presente en el entorno. 

Imaginate que estás en un concierto y que tu acompañante te está gritando al oído. Si tu amigo grita a -60dBm y la música está a -90dBm, la relación señal-ruido (SNR) es de 30dB, lo que significa que la señal de tu amigo es 1000 veces más fuerte que el ruido de fondo, por lo que podrás escuchar a tu amigo sin problemas.  Sin embargo, si la música está a -70dBm, la SNR es de 10dB, lo que significa que la señal de tu amigo es solo 10 veces más fuerte que el ruido de fondo, por lo que tendrás dificultades para escuchar a tu amigo debido al ruido de fondo.

**En una red wifi se considera que una SNR de 20dB o más es buena, mientras que una SNR de 10dB o menos es mala y puede resultar en una conexión inestable o lenta.**



!!! task "A209(CE3,CE5) Auditoría Wi-Fi"

    En está actividad, deberás auditar la calidad de la red inalámbrica 'WIFI_EDU'. Debes identificar qué AP da servicio en cada zona y si la calidad de la señal es suficiente.

    Instrucciones:

    **1. Preparación**: Instala en tu dispositivo la app *Wifi Analyzer, Wifi Heatmap* o cualquier otra app de análisis de redes Wi-Fi que te permita medir la intensidad de la señal y la calidad de la red inalámbrica.

    **2. Trabajo de campo**: Recorre las zonas asignadas en el plano. En cada punto de medición, debes localizar la señal con el ESSID `WIFI_EDU`

    **3. Identificación única**: Ten en cuenta que verás varias señales llamadas igual. Debes fijarte en el BSSID para saber exactamente a qué antena física te estás conectando.

    **4. Toma de datos**: Completa la siguiente tabla.

    |Ubicación (Planta, pasillo, aula)| BSSID (MAC del AP)| Frecuencia (Ghz)| Canal | Intensidad (dBm) | SNR (dB) | Calidad de la señal (Buena, Regular, Mala) |
    |---------------------------------|-------------------|-----------------|-------|------------------|----------|---------------------------------------------|
    |                                 |                   |                 |       |                  |          |                                             |

    **5. Plano de calor**: Con los datos obtenidos, elabora un plano de calor de la intensidad de la señal Wi-Fi en cada una de las zonas asignadas.

### 4.3 Seguridad en redes WiFi

#### 4.3.1 Redes Abiertas

Estas redes se caracterizan por no tener implementado ningún sistema de autenticación o cifrado. Las comunicaciones entre los terminales y los AP viajan en texto plano (sin cifrar) y no se solicita ningún dato para acceder a la red.

Existe la creencia errónea de que **ocultar el ESSID** de la red Wi-Fi o **filtrar las conexiones por dirección MAC** son medidas de seguridad suficientes, pero la realidad es que ninguna de ellas ofrece una protección real. Al no existir cifrado en las comunicaciones entre los terminales y el AP, cualquier atacante que utilice herramientas de auditoría de redes inalámbricas podrá no solo descubrir la existencia de la red, sino también interceptar y capturar el tráfico que circula entre los clientes y el punto de acceso.

#### 4.3.2 WEP (Wired Equivalent Privacy)

WEP se introdujo en 1997 y fue el primer intento de protección inalámbrica. El objetivo era cifrar la comunicación entre los dispositivos para que en el caso de que fuera la comunicación interceptada por un atacante, este no pudiera entender el contenido de la comunicación. 

WEP utiliza el algoritmo de cifrado RC4, que se demostró que era vulnerable a **ataques de fuerza bruta**. A pesar de los intentos de dificultar la tarea de los atacantes aumentando el tamaño de la clave de cifrado, el posterior aumento de la potencia informática hizo que los ataque de fuerza bruta fueran cada vez más efectivos. 

#### 4.3.3 WPA (Wi-Fi Protected Access)

La urgencia de la situación hizo que en 2003 apareciera en escena el protocolo WPA.

WPA utilizaba el llamado **Temporal Key Integrity Protocolo (TKIP)**, consistente en generar una nueva clave de cifrado para cada paquete de datos transmitido, lo que dificultaba enormemente los ataques de fuerza bruta. TKIP se implementó como una solución temporal para mejorar la seguridad de las redes inalámbricas sin necesidad de reemplazar el hardware existente, ya que era compatible con los dispositivos que utilizaban WEP.

A pesar de las mejoras introducidas por WPA, se descubrió que TKIP también tenía vulnerabilidades, lo que llevó al desarrollo de WPA2 en 2004.

#### 4.3.4 WPA2

WPA2 utiliza el algoritmo de cifrado AES (Advanced Encryption Standard). Tiene dos posibles modos de funcionamiento:

- **Modo personal o clave precompartida (PSK - Pre-Shared Key)**: se basa en un código de acceso compartido y generalmente se usa en entornos domésticos o pequeñas oficinas.
  
![WPA2-PSK](assets/wpa_psk.jpg){ .center width="40%" }

- **Modo empresarial (WPA-EAP)**: se basa en un servidor de autenticación (RADIUS) para gestionar el acceso a la red y se utiliza en entornos empresariales o corporativos.

#### 4.3.5 WPA3

En 2018, se introdujo el protocolo WPA3, que es la última versión del estándar de seguridad para redes Wi-Fi. WPA3 ofrece mejoras significativas en la seguridad de las redes inalámbricas. 

Al igual que WPA2, WPA3 tiene dos modos de funcionamiento: el modo personal (WPA3-PSK) y el modo empresarial (WPA3-EAP).

!!! task "A210.1(CE3,CE5) - Configuración de puntos de acceso inalámbricos *Legacy*"

    En esta actividad, deberás configurar un punto de acceso inalámbrico *legacy* (antiguo). Para ello utilizaremos un emulador de router inalámbrico Linksys [WRT300N](https://ui.linksys.com/LegacyRouters/WRT300N/0.93.9), que es un router doméstico que se comercializó en 2007 y que soporta los protocolos de seguridad WEP y WPA.

    Como prueba de realización de la actividad deberás proporcionar en un documento de texto con capturas de pantalla de cada uno de los siguientes apartados:

    1. Crea una red inalámbrica con ESSID `WIFI_XXX`donde XXX son las iniciales de tu nombre y apellidos. Los canales los dejamos en automático y activamos la difusión del ESSID para que la red sea visible. En seguridad inalámbrica seleccionamos en *Security Mode* la opción `Disabled`. De esta forma hemos creado una red inalámbrica abierta y sin cifrado, es decir, sin ninguna medida de seguridad.

    2. Para crear una red con autenticación segura y transmisión cifrada, seleccionamos en Security Mode la opción de **WPA2 Personal**, encriptación **AES**, dejamos la **renovación de claves de cifrado cada hora** y ponemos la **clave `nopuedo`** como frase para autenticarnos en nuestra red.


!!! task "A210.2(CE3,CE5) - Configuración de puntos de acceso inalámbricos 2"

    En esta actividad deberás configurar un router inalámbrico moderno. Para ello utilizaremos el emulador de router inalámbrico [Tp-link AX3000](https://emulator.tp-link.com/ax55v4_ap/index.html?_gl=1*qi6jns*_gcl_au*MTUzMzc2ODUwMy4xNzczOTQwNTcx*_ga*MTEyNDQxMjY0Ny4xNzczOTQwNTcx*_ga_X5XJFE5K24*czE3NzM5NDA1NzAkbzEkZzEkdDE3NzM5NDA4NjQkajYwJGwwJGgw#networkMap)

    **1. Identidad y Visibilidad**  

    - **Cambia el SSID de las redes inalámbricas 2,4GHz y 5GHz a `WIFI_XXX` y `WIFI_XXX_5G`** respectivamente, donde XXX son las iniciales de tu nombre y apellidos.
    - **Desactiva la opción *Smart Connect*** para poder gestionar las bandas de forma independiente.
    - **Activa la casilla Hide SSID** en la red de 5GHz y explica qué implica esto para un usuario nuevo que intenta conectarse.

    **2. Fortificación del Acceso**

    - Configura **la red de 2,4GHz con el protocolo de seguridad WPA3 Personal**, encriptación AES. **Genera una contraseña segura** de al menos 12 caracteres, que incluya mayúsculas, minúsculas, números y caracteres especiales. Explica por qué es importante utilizar una contraseña fuerte en una red Wi-Fi. **Selecciona automática del canal de comunicación y modo de funcionamiento mixto (b/g/n/ax)**.   
    - Configura **la red de 5GHz** con el protocolo de seguridad** WPA3+WPA2 Personal**, encriptación AES. Genera una **contraseña segura**. **Selecciona automática del canal de comunicación**. Configura el** modo de funcionamiento en exclusiva para dispositivos Wi-Fi 6 (ax)**.
    
    
Recursos:

- [Kaspersky. WEP, WPA, WPA2 y WPA3](https://www.kaspersky.es/resource-center/definitions/wep-vs-wpa)

- [Incibe - WPA3: el protocolo de seguridad a configurar en tu router](https://www.incibe.es/ciudadania/blog/wpa3-el-protocolo-de-seguridad-configurar-en-tu-router)

- [Xataka - Caos en la seguridad WiFi: un repaso a las vulnerabilidades de WEP, WPA y WPA2](https://www.xataka.com/seguridad/caos-en-la-seguridad-wifi-un-repaso-a-las-vulnerabilidades-de-wep-wap-y-wap2)
