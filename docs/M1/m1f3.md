...
**Lotes (batch size)**: un *lote* es una porción del conjunto de datos que se utiliza para entrenar el modelo en una iteración. En lugar de procesar todo el conjunto de datos a la vez, se divide en lotes más pequeños. Esto ayuda a mejorar la eficiencia del entrenamiento y a reducir el uso de memoria.
...
**Cantidad de épocas (epochs)**: una *época* se refiere a una pasada completa por todo el conjunto de datos de entrenamiento. Durante una época, el modelo ve y aprende de todas las muestras en el conjunto de datos una vez.
...
**Tasa de aprendizaje (learning rate)**: determina la magnitud de los ajustes que el modelo hace en cada paso. Es, en esenciam la velocidad a la que aprende.
...
- **Precisión por época**. Mide qué tan **bien** el modelo está clasificando las imágenes correctamente en cada época de entrenamiento. Un valor alto cercano a 1.0 (100%) indica que el modelo está haciendo predicciones correctas la mayoría de las veces.

- **Pérdida por época**. Mide que tan **mal** lo está haciendo el modelo. Un valor bajo cercano a 0.0 indica que el modelo está cometiendo pocos errores en sus predicciones.

Estas gráficas son la herramienta perfecta para diagnosticar el **sobreajuste (overfitting)** y el **subajuste (underfitting)**.



**Precisión por clase**

- **Matriz de confusión**. Esta tabla es un diagnóstico detallado de los aciertos y errores del modelo **para cada categoría**. Es extremadamente útil para ver **dónde se está equivocando** el modelo.
