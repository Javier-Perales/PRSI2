...
Sesión 7: ¿Cómo lo Arreglamos? El Papel del Software Libre.

    Actividad (30 min): Se plantea cómo podrían "arreglar" su modelo (la respuesta es: con un dataset más diverso y representativo).

    Conexión con Saberes (30 min): Se introduce el concepto de "Comunidades de desarrollo de software libre". Se debate:

        ¿Cómo ayuda la transparencia (poder ver el código o incluso los datos de entrenamiento) a detectar sesgos?

        ¿Cómo el tener equipos de desarrollo más diversos puede prevenir que se introduzcan estos sesgos desde el principio?

        Se menciona que modelos como MobileNet (el que usa Teachable Machine por debajo) son a menudo de código abierto, lo que permite a la comunidad estudiarlos y mejorarlos.

Sesión 8: Conclusiones y Manifiesto de IA Ética.

    Actividad (60 min): Como proyecto final del sprint, la clase entera colabora para redactar un "Manifiesto para el Desarrollo de Software Inclusivo". Este documento recogerá los aprendizajes del sprint en forma de buenas prácticas.

        Ej: "Nuestros datos de entrenamiento deben reflejar la diversidad de la sociedad".

        Ej: "Debemos testear nuestros modelos con casos que desafíen nuestras asunciones".

    Esta actividad final consolida el aprendizaje, fomenta el trabajo en equipo y genera un resultado tangible que demuestra la adquisición de la competencia CE5.
