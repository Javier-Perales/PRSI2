...
!!! note "Fase 2: Adquirir Datos Nuevos"
    ...
    - `convert("RGB")` se usa para asegurarse de que la imagen está en formato RGB (3 canales de color).
...
!!! note "Fase 3: Pre-procesamiento"
    ...
    - **Normalizar los valores de los píxeles (ej. de [0, 255] a [-1, 1]).** Para ello deberemos utilizar la librería *Numpy* (`ìmport numpy as np`)

    ```python
    # Convertimos la imagen en un array NumPy.
    imagen_array = np.asarray(imagen)
    ```
    De esta forma, cada píxel de la imagen pasa a representarse como un **número entero** (Los valores estarán entre 0 y 255) dentro de una matriz tridimensional. Recueda que una imagen *RGB*, cada pixel está codificado por tres valores (*Red*, *Green* y *Blue*). Por tanto, si ejecutamos `image_array.shape`, saldrá (224,224,3)
...
!!! question "A106.1 Exportación de un modelo desde Teachable Machine"
...
## Sesión 3,4,5,6: Evaluación en lote
...
!!! question "A106.2 Preparación del Lote y refactorizar el codigo (1ª parte)"

    1. En Google Drive, debemos tener una carpeta `test` que contiene las imágenes de prueba.
    2. Vamos a crear una variable que contenga un array con el nombre de cada imagen de prueba. Para ello, usaremos la librería `os` de Python.
    ...
    ```
    4. Ahora deberíamos recorrer el array `lista_archivos` y para cada imagen, ejecutar las fases de inferencia vistas en la sección anterior. 
    ...
    5. Deberás desarrollar la función `predecir_imagen(ruta_imagen)` que implemente las fases de inferencia vistas en la sección anterior. Esta función debe recibir la ruta de una imagen, procesarla y devolver la etiqueta predicha y la probabilidad asociada.


## Sesión 7,8: Publicación de resultados (*Github*)
...
!!! question "A106.3 - Cálculo de métricas y generación del informe"

    1. Calculamos la precisión del modelo y la probabilidad media de aciertos.
    ...
    for error in predicciones_incorrectas:
        informe += f"Archivo: {error['archivo']}, Predicción: 
            {error['prediccion']}, Probabilidad: {error['probabilidad']:.2%}\n"

    print(informe)
    ```

Github surgió como  una plataforma de desarrollo colaborativo de software para alojar proyectos utilizando el sistema de control de versiones Git. Permite a los desarrolladores trabajar juntos en proyectos, gestionar cambios en el código fuente y colaborar de manera eficiente.

En nuestro caso, vamos a usar GitHub para publicar el cuaderno *Jupyter* generado con *Google Colab* y compartir nuestro proyecto con la comunidad. Básicamente, utilizaremos GitHub como **portafolio profesional**.

!!! question "A106.4 - Publicación en GitHub"

    Para conectar Google Colab con GitHub, necesitas dominar solo tres términos básicos. No necesitas memorizar comandos complejos todavía, solo entender la lógica:

    #### El Repositorio (Repo)

    Es como una "carpeta de proyecto". Contiene todos los archivos de tu proyecto (código, imágenes, documentación) y, lo más importante, el historial de cambios de cada archivo.

    **- Crea un repositorio con un nombre suficientemente significativo, relacionado con el contenido.**

    En la creación del repositiorio, indica una descripción breve del proyecto y selecciona la opción de inicializar el repositorio con un archivo **README.md**.

    El archivo `README.md` es un archivo de texto en formato Markdown que sirve como la página principal de tu repositorio. Aquí puedes describir el propósito del proyecto, cómo usarlo, cómo contribuir, etc. En nuestro caso, lo usaras para realizar una descripción general del proyecto que has desarrollado en Google Colab.

    Los repositorios pueden ser **públicos** (cualquiera puede verlos) o **privados** (solo tú y las personas que invites pueden verlos). Para un portafolio profesional, generalmente querrás que sean públicos. 

    #### Commit (Confirmación)

    Un *commit* no es solo "guardar". Es sacar una fotografía exacta de tu código en un momento concreto.

    Cada *commit* tiene un identificador único y debe llevar un mensaje explicativo (ej: "Añadida función para calcular números primos").

    #### Rama (Branch)

    Esto nos permite "viajar en el tiempo": si rompes algo hoy, puedes volver a la foto exacta de ayer. Esto es parte de las herramientas de mantenimiento y optimización de software.

    magina una línea de tiempo. La línea principal se llama **main**.

    Todos los cambios que aprobemos irán a esta rama principal. Por ahora, trabajaremos siempre sobre la rama main para mantener nuestro portfolio sencillo y lineal.


    #### De Google Colab a GitHub

    Google Colab tiene una integración nativa que facilita enormemente la gestión documental. 

    El proceso conceptual que realizaremos en cada práctica es el siguiente:

    - Codificación: Creas y pruebas tu código Python en los cuadernos de Google Colab .

    Vinculación: En el menú de Colab, seleccionas Archivo > Guardar una copia en GitHub.

    - Autorización: La primera vez, tendrás que dar permiso a Google para escribir en tu GitHub.

    - Publicación (El *Commit*):

        * Seleccionas tu repositorio.

        * Escribes el mensaje del *commit*.

        * Marcas la opción "Incluir un enlace a Colaboratory". Esto crea un botón en tu GitHub para que cualquiera (incluido yo para corregir) pueda abrir y ejecutar tu código con un clic.
    ...
    3. Publicamos el informe en un repositorio de GitHub.

    - Crea un nuevo repositorio en GitHub.
    - Sube el cuaderno de Google Colab y el informe generado.
    - Añade una descripción detallada del proyecto, incluyendo los pasos seguidos, las fases de inferencia implementadas y los resultados obtenidos.
